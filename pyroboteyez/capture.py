...
class App():
    def __init__(self, window, window_title, video_source=0, no_flip=False,
        ...
        self.vid = Capture(self.video_source, focus=focus, width=width,
                           height=height)
        self.canvas = tk.Canvas(window, width = self.d_w, height = self.d_h)
        self.canvas.pack()
    ...
    def update(self):
        ...
        if ret:
            ...
            frame=cv2.resize(frame, (self.d_w, self.d_h))
            cv2.waitKey(1)
...
if __name__=='__main__':           
    ...
    parser.add_argument('-w',"--width", help="width of the image", type=int, default=2304)
    parser.add_argument('-h',"--height", help="height of the image", type=int, default=1536)
